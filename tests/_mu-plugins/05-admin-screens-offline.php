<?php
/**
 * Plugin Name: Paytrail Tests, quiet wp-admin screens
...
// Paytrail still has to be reachable, so only the hosts the suite never needs are cut.
add_filter('pre_http_request', static function ($preempt, $args, $url) {
    $host = (string) parse_url($url, PHP_URL_HOST);

    if (preg_match('/(^|\.)(wordpress\.org|woocommerce\.com|gravatar\.com)$/', $host)) {
        return new WP_Error('paytrail_tests_offline', 'Blocked in tests: ' . $url);
    }
