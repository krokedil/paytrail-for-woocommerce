...
trait CanFakeSubscriptions {
	...
	/**
	 * Forgets every faked subscription. Static state, so this runs between tests
	 * whether or not the test used the fixtures.
	 */
	protected function resetFakeSubscriptions(): void {
		SubscriptionsRegistry::reset();
		delete_option( \WC_Subscriptions_Admin::$option_prefix . '_accept_manual_renewals' );
	}
}
