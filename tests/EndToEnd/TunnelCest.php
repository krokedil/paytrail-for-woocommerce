...
/**
 * A browser reaches the tunnel once per purchase, on the return URL Paytrail sends the
...
class TunnelCest
{
	...
	{
		$public = rtrim((string) ($_ENV['PAYTRAIL_WORDPRESS_URL'] ?? ''), '/');
		$local  = rtrim((string) ($_ENV['WORDPRESS_URL'] ?? ''), '/');

		Assert::assertNotSame('', $public, 'PAYTRAIL_WORDPRESS_URL is not set in tests/.env.');
		Assert::assertNotSame('', $local, 'WORDPRESS_URL is not set in tests/.env.');
		Assert::assertNotSame($public, $local, 'PAYTRAIL_WORDPRESS_URL and WORDPRESS_URL are the same host.');

		// With a query string, which Paytrail's return URL carries the order key in.
		$path = '/shop/?paytrail-tunnel-probe=1';
